...
<div class="contact-page">
  <header><p class="muted"><a href="/admin">Home</a> / Contact Inbox</p><h1>Contact Inbox</h1></header>
  {#if data.loadError}
    <p class="notice error" role="alert">Inbox belum bisa dimuat. <a href="/admin/contact">Coba lagi</a>.</p>
  {:else}
    <section class="panel" aria-label="Pesan masuk">
      {#each data.inquiries as inquiry (inquiry.id)}
        <a class="row" href={`/admin/contact/${inquiry.id}`}>
