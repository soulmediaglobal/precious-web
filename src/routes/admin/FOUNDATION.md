...
Ray explicitly approved the shared hook matcher correction: /admin and /admin/* remain protected; /admin is an unauthenticated static shell in Phase 1. Do not add data or deploy as a functional CMS before later auth work. Route /admin and this approved hook boundary need Nurey canonical documentation sync; canonical files were not edited here.
...
- `/admin/login` is public (the exact login path is exempt from the v2 guard). An already authenticated visitor is redirected to `/admin`; successful email/password login also redirects there.
- `/admin` and `/admin/*` are protected except for the exact `/admin/login` path. The shared server hook checks `locals.getUser()` and redirects unauthenticated requests to `/admin/login`.
- Logout uses a form POST to `/admin/logout`, calls the existing Supabase Auth `signOut()`, and redirects to `/admin/login` on success. There is no GET logout handler; a returned sign-out error produces a generic 503 response.
...
Phase 5 provides `/admin/team`, `/admin/team/new` and `/admin/team/[id]`, a shared v2 Team form, and Team-aware sidebar navigation. It reuses the existing `parseTeamForm` / `uploadTeamImage` helpers and database query layer, with Team create/update/delete helpers added to `src/lib/server/db/queries.ts`. Clients and Projects remain placeholders; the Sales Dashboard remains static sample content. This is not completion of the broader CMS.
