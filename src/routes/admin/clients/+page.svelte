...
<svelte:head><title>Clients | Precious Admin</title></svelte:head>

<div class="client-page">
  <header class="client-heading">
    <div>
      <p class="breadcrumb"><a href="/admin">Home</a> / Clients</p>
      <h1>Clients</h1>
      <p class="intro">Manage project clients and contacts.</p>
    </div>
    <a class="button primary" href="/admin/clients/new">Add client</a>
  </header>
          ...
          <tbody>
            {#each data.clients as row (row.client.id)}
              <tr>
                ...
                <td>
                  <div class="actions">
                    <a class="button" href={`/admin/clients/${row.client.id}`} aria-label={`Edit ${row.client.companyName}`}>Edit</a>
